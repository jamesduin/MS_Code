...
coarse-grained classifiers and active learning outperforms passive learning. Passive learning is
randomly selecting new data instances to be labeled out of the set of unlabeled instances.
 ...
 dataset \cite{yugi}. Futhermore, the fine-grained classifier is shown to have a higher level of
 precision compared to the coarse-grained classifier when both classifiers output the roughly the
 same recall  \cite{yugi}. These evalutation metrics are discussed futher in
...
of negative instances in the subset paritioning is 3827 down from 19136. The total number of
positive instances in the subset paritioning stays the same and is 962.
...
The default gamma setting is 0.002967 or (1/num-features) or (1/337). Default
 ...
 classifier are used in the SVM classifier instead of the SVM's default balanced option. The fine-grained
  ...
  the coarse-grained classifer which is 0.903. Thus, the SVM parameters of Cost 0.15 and Gamma 0.002
...
\par This chapter presents the experimental results for this work. We start by analysing
