  ...
  \par This study investigates an application of the Support Vector Machine
   ...
   according to a
    ...
    We show that purchasing fine-grained labels in each round of active learning produces a
      ...
      labels for this dataset, so multiple cost ratios ran and an optimal FFR
      purchasing strategy is determined for each fine cost.
\end{abstract}
...
follows Mo et al.'s \cite{yugi} experiments on the Reuters Corpus Volume~I (RCV1)
 text categorization datset shown in section \ref{sect:activeOverLabel}. HAL is
...
options of using sample weights and dropping itermediate values are discovered to have the
highest performance.
...
The fine weight of 23 is discoved to have the highest performance when the entire dataset is used.
...
demonstrate a prominent advantage for active fine with the Logit classifier. Furthermore
 ...
 that for a budget of aroun 20-60 rounds a FFR strategy of 0.2 would have the highest performance,
