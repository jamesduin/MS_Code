...
dominating after round 10 while the Active Fine ROC curve is still well below the
 Active Coarse at that round. The experiment is ran again with the Fine classifier
  ...
  \textit{Figure \ref{fig:LogReg4plots}}. These results show that the expected
...
\begin{figure*}[!htb]
    ...
    \begin{subfigure}[t]{0.65\textwidth}
        \centering
        \includegraphics[width=\textwidth]{fig/resultsrunActPassLogReg4plots_pr}
        \caption{LogReg PR Curve Analysis}
    \end{subfigure}% %~

    \begin{subfigure}[t]{0.65\textwidth}
        \centering
        \includegraphics[width=\textwidth]{fig/resultsrunActPassLogReg4plots_roc}
        \caption{LogReg ROC Curve Analysis}
    \end{subfigure}
    \caption{Note the Active Fine PR curve red line intersects with the Coarse
