...
\begin{acknowledgments}
  ...
  research topic, and Yuji Mo and Dr. Douglas Downey for their work in developing the HAL and
...
how the new data instances will affect classifier performance \cite{mitchell}.
 Although the gains seen by using an active learning approach will be dependent upon
 the learning model and the dataset, a survey by Tomanek et al. \cite{TomanekActive} reports
  that $91$\% of researchers who used active learning in large-scale
  annotation projects had their expectations fully or partially met. Furthemore, Dagsputa
  \cite{Dasgupta2004} provided a variety of theoretical upper and lower bounds for active
  learning when batches of instances are purchased at a time and a linear classifier is used,
   showing that in the worst case active learning is equivalent to passive supervised
   learning. See Settles \cite{settles1995active} for a more formal empirical and
   theoretical analysis of the advantages of active learning. Active learning
...
can also learn a best performing classifier for a given purchase budget. We work with the
pool-based active learning model where batches of instances are purchased at a time using
the previous best performing classifier to determine which instances to purchase for the next
batch that will best improve classifier performance.
...
disease protein dataset \cite{yuji}.
...
purchasing strategy is determined for each fine cost. An approach optimally selecting
 FFR strategies throughout the rounds using a multi-armed bandit strategy is also applied to the protein dataset
 and shown to be robust to variations in labeling cost and budget. The main contribution of this thesis is applying the
  multi-armed bandit active over-labeling approach to the protein dataset and demonstrating that this approach
...
% A separate classifier is trained for each class in the protein dataset.
...
Precision is a measure of result relevancy and is given as: %in \textit{eqn. \ref{eq:precision}}.
\FloatBarrier
\begin{equation}
\label{eq:precision}
\begin{aligned}
...
Recall is a measure of how many truly relevant results are returned and is given as: %in \textit{eqn. \ref{eq:recall}}.
\FloatBarrier
\begin{equation}
\label{eq:recall}
\begin{aligned}
...
The F-measure or F1-measure (F1) is the harmonic mean of precision and recall and is given as: %in \textit{eqn. \ref{eq:fmes}}.
\FloatBarrier
\begin{equation}
\label{eq:fmes}
\begin{aligned}
...
(non-root) concepts and combining the results to predict the coarse-grained (root) label \cite{yuji}.
...
\ref{fig:union}} from \cite{yuji}. The classifiers for this dataset can be thought of as a
...
\begin{figure}[!htb]
    ...
    learners for each circle type, from \cite{yuji}.}
    \label{fig:union}
\end{figure}
\FloatBarrier
...
%one, instead of passively being given a training set \cite{mitchell}.
...
that no value at that level applies \cite{yuji}.
...
classifiers is given in as: %\textit{eqn. \ref{eq:maxcombine}}.
...
uncertainty $u_i(x)$ of the label for example $x$ for level $i$ is defined as: %\textit{eqn. \ref{eq:uncert}}.
\begin{equation}
\label{eq:uncert}
...
fine-grained ratio (FFR) \cite{yuji}.
...
dynamically adapt to purchase proportions \cite{yuji}. The task of choosing the
...
observed model change and the equation is given below, where %in \textit{eqn. \ref{eq:reward}}. Where
...
proportions to use for HAL \cite{yuji}. See text for
$\epsilon$-greedy bandit algorithm, HAL, Gain, and Reward.
}
\label{alg:bandit}
\begin{algorithmic}
\Function{BANDIT}{Unlabeled examples $U$, labeling tree $T$,
budget $B$, per-iteration budget $b$, purchase proportions
 $p = (p_1,\ldots,p_k)$, Arms=$\left \{armStay,armSwitch \right \}$, HAL}
...
\thispagestyle{empty}
\section{Previous work in Active Learning}
...
can improve the accuracy of a coarse-grained (root-level) classifier, and
 ...
 label acquisition cost can vary \cite{yuji}. This thesis mirrors the first
 application of these methods done by Mo et al. \cite{yuji}.
...
follows Mo et al.'s \cite{yuji} experiments on the Reuters Corpus Volume~I (RCV1)
 text categorization dataset shown in Section \ref{sect:activeOverLabel}. HAL is
 applied to a Dispatch dataset by Mo et al. \cite{yuji}. This dataset contains 375,026 manually labeled
...
randomly selecting new data instances to be labeled out of the set of unlabeled instances.
 ...
 dataset \cite{yuji}. Furthermore, the fine-grained classifier is shown to have a higher level of
 precision compared to the coarse-grained classifier when both classifiers output the roughly the
 same recall  \cite{yuji}. These evaluation metrics are discussed further in
 Section \ref{sect:PerformanceMetrics}.

%These results are shown in \textit{Figure \ref{fig:draft-RCV1}}.
%\FloatBarrier
%\begin{figure}[!htb]
%	\centering
...
Mo et al. \cite{yuji} demonstrate that HAL can be used to determine an optimal FFR strategy
...
\section{Varying SVM Scaling Methods}
...
\section{Varying SVM Feature Selection}
...
\section{Varying Logistic Regression Scaling}
...
\section{Varying Logistic Regression Feature Selection}
...
\section{Varying Logistic Regression Positive Class Weight and Cost}
...
\section{Varying Logistic Regression Fine Class Weights}
...
\subsection{Tune Fine Class 1 Weights}
...
\subsection{Tune Fine Class 2 Weights}
...
\subsection{Tune Fine Class 3 Weights}
...
\subsection{Tune Fine Class 4 Weights}
...
\subsection{Tune Fine Class 5 Weights}
...
\subsection{Tune Fine Class 6 Weights}
...
\subsection{Tune Fine Class 7 Weights}
...
\subsection{Tune Fine Class 8 Weights}
...
\section{Varying Logistic Regression Tolerance}
...
\section{Varying Sample Weight On Test Set and Dropping Intermediate ROC Curve Values}
...
\section{Varying Logistic Regression Positive Class Weight for Full Dataset}
...
\section{Varying SVM Cost and Gamma}
\label{sect:paramEnd}
...
The default gamma setting is $0.002967 \approx 0.003$  or (1/num-features) or (1/337). Default
...
\begin{table}[H]
\centering
\caption{SVM Cost 1.0 Gamma 0.003}
\label{tab:SVM-C1-Gp003}
\begin{tabular}{|l||l||l||l||l||l||l|}\toprule
Title & PR & ROC & Acc & F1 & conf (tn/fn) & conf (fp/tp) \\ \midrule
...
\begin{table}[H]
\centering
\caption{SVM Cost 2.0 Gamma 0.003}
\label{tab:SVM-C2-Gp003}
\begin{tabular}{|l||l||l||l||l||l||l|}\toprule
Title & PR & ROC & Acc & F1 & conf (tn/fn) & conf (fp/tp) \\ \midrule
...
\begin{table}[H]
\centering
\caption{SVM Cost 0.1 Gamma 0.003}
\label{tab:SVM-Cp1-Gp003}
\begin{tabular}{|l||l||l||l||l||l||l|}\toprule
Title & PR & ROC & Acc & F1 & conf (tn/fn) & conf (fp/tp) \\ \midrule
...
\begin{table}[H]
\centering
\caption{SVM Cost 0.05 Gamma 0.003}
\label{tab:SVM-Cp05-Gp003}
\begin{tabular}{|l||l||l||l||l||l||l|}\toprule
Title & PR & ROC & Acc & F1 & conf (tn/fn) & conf (fp/tp) \\ \midrule
...
\begin{table}[H]
\centering
\caption{SVM Cost 0.15 Gamma 0.003}
\label{tab:SVM-Cp15-Gp003}
\begin{tabular}{|l||l||l||l||l||l||l|}\toprule
Title & PR & ROC & Acc & F1 & conf (tn/fn) & conf (fp/tp) \\ \midrule
...
\begin{table}[H]
\centering
\caption{SVM Cost 0.2 Gamma 0.003}
\label{tab:SVM-Cp2-Gp003}
\begin{tabular}{|l||l||l||l||l||l||l|}\toprule
Title & PR & ROC & Acc & F1 & conf (tn/fn) & conf (fp/tp) \\ \midrule
