...
clf = joblib.load('coarse_models/coarse_fold_1.pkl')
...
for estimate in confEstimate:
    if X_train_pred[count] == 1 or y_trainCoarse[count] == 1:
        #print("%s %s %s" % (estimate, y_trainCoarse[count],X_train_pred[count]))
        print("%s %s" % (estimate, X_train_pred[count]))
    count += 1
print(confEstimate.shape)

print(clf.decision_function(X_train[i]))