...
for part in partitions:
    ...
    for i in sorted(folds):
        ...
        with open('../data/partitionMinMaxScaled/partitionMinMaxScaled_' + str(i)) as f:
        #with open('../data/partitionStdSclSel/partitionStdSclSel_' + str(i)) as f:
            for line in f:
                nums = line.split()
    ...
    for lvl in rnds:
        ...
        for testFold in fold_list:
            ...
            fileName = open('results/coarse_' + str(part) + '.res', 'wb')
            pickle.dump(results[part]['coarse'], fileName)
            fileName.close()
            fileName = open('results/fine_' + str(part) + '.res', 'wb')
            pickle.dump(results[part]['fine'], fileName)
            fileName.close()
