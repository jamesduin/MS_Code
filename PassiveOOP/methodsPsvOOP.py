...
def printConfMatrix(y_testCoarse,y_predCoarse,f):
    print("cumulative")
    f.write("cumulative\n")
...
def plotRocPrCurves(y_testCoarse,y_pred_score,y_sampleWeight,lvl,fName,testFold,results):
    ##### Plot roc_auc
    ...
    plt.savefig(lvl + '_results/' + fName + '_ROC_' + str(testFold) + '.png')
    ...
    plt.legend(loc="lower right")
    plt.savefig(lvl + '_results/' + fName + '_PR_' + str(testFold) + '.png')
    results.append([str(testFold)] + [roc_auc] + [average_precision])
...
def saveResults(lvl,results_fine,f,start_time):
