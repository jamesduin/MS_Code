...
def main():
    ...
    for cost in sorted(costs):
        print(cost)
        resultsDir = 'BANDIT_'+str(cost).replace(".","p")+'/results'
        print(resultsDir)
        resultAtRound,mixedCostPRAtCost = printBanditPlot(resultsDir, cost)

        resultAtRound = np.array(resultAtRound).reshape(1,48)
        if len(banditRes) == 0:
            banditRes = resultAtRound
        else:
            banditRes = np.vstack((banditRes,resultAtRound))
        print('mixedCostPRAtCost {} shape {}'.format(cost,mixedCostPRAtCost.shape))
        if len(mixedCostPR) == 0:
            mixedCostPR = mixedCostPRAtCost
        else:
            mixedCostPR = np.hstack((mixedCostPR,mixedCostPRAtCost))

    print(banditRes)
    fileName = open('BanditRndSel_'+str(rndSel)+'.res','wb')
    pickle.dump(banditRes.tolist(),fileName)
    fileName.close()

    print('mixedCostPR shape {}'.format(mixedCostPR.shape))
    fileName = open('mixedCostPR.res', 'wb')
    pickle.dump(mixedCostPR, fileName)
    fileName.close()


def printBanditPlot(resultsDir,cost):
    mixedCostPRAtCost = []
    resultAtRound = []
    Xlims = [0, 180]
    ...
    for linInd,type in enumerate(sorted(rndTypeSet)):
        ...
        y_pr = np.array(y_pr).reshape(len(y_pr), 1)
        if len(mixedCostPRAtCost) == 0:
            mixedCostPRAtCost = y_pr
        else:
            minRL = min(len(mixedCostPRAtCost), len(y_pr))
            mixedCostPRAtCost = np.hstack((mixedCostPRAtCost[:minRL], y_pr[:minRL]))
        print('mixedCostPRAtCost shape {}'.format(mixedCostPRAtCost.shape))
        print('y_pr shape {}'.format(y_pr.shape))

        #cVal = tuple(np.multiply(cVals[linInd],(1/270,1/270,1/270,1.0)))
        cVal = cVals[linInd]
        #print('FFR: {}'.format([type,x_pr[rndSel-1],cost,y_pr[rndSel-1]]))
        resultAtRound.append([type,float(x_pr[rndSel-1]),float(cost),float(y_pr[rndSel-1])])
    ...
    plt.close()
    return resultAtRound, mixedCostPRAtCost
