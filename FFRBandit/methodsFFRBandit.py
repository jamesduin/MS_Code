...
def getUnlabeledX(results,classes_all):
    data = []
    for x in sorted(classes_all):
...
def appendRndTimesFoldCnts(testFold, rndNum, results, sets,classes_all, start_time):
    instanceCount = dict()
    for lvl in ['coarse', 'fine']:
        instanceCount[lvl] = 0
    ...
    instanceCount['X_unlabeled'] = 0
    fold_cnt = ['rnd'] + [rndNum] + ['fold'] + [testFold] + ['lvl'] + ['X_unlabeled']
    for i in sorted(classes_all):
        instanceCount['X_unlabeled'] += len(classes_all[i])
        fold_cnt.append((i, len(classes_all[i])))
    fold_cnt.append(('tot', instanceCount['X_unlabeled']))
    addPrint(results, fold_cnt)
